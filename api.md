...
> ⚠️ **安全说明**：
> - **Web 服务**默认绑定所有网络接口（局域网内任何设备可访问），可在「设置 → 其他 → Web 访问令牌」配置令牌后，要求请求携带 `Authorization: Bearer <token>` 或 `?token=<token>`。**仍建议仅在可信局域网内开启**，切勿在公网或不可信网络暴露。
> - **Content Provider**（`readerProvider`）要求调用方声明 `io.legado.READ_WRITE` 权限。
> - **书源为第三方脚本**：书源规则使用 JavaScript（Rhino）执行、网页经系统 WebView 加载、网络请求可指向任意 URL（含内网地址）。导入未知来源书源等同运行第三方代码，请仅导入可信书源。
