...
- allowed_external_providers: 当前 mode=dsh-claude 只授权 dsh 与 claude 两个提供方接收本项目内容；codex 未纳入路由，不得外发；扩大范围必须先改本文件并留痕
- local_agent_presets_or_tools: 已授权本地代理 dsh（人类启动，当前 controller/executor/acceptor）与 claude（reviewer，只读）；本地工具链 git、Windows PowerShell 5.1/pwsh、Gradle wrapper（须先设 GRADLE_USER_HOME=D:\gradle_home）、Android SDK/adb 与真机 10CE5P1M1Z001P9、Python 造测试书脚本
